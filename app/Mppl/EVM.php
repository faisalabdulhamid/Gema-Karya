...
class EVM 
{
    ...
    public function getActivity()
    {
    	...
    	foreach ($pekerjaan as $key => $value) {
    		...
    		$kegiatan['bobot'] = $value['bobot'];

    		$kegiatan['id'] = $value['id'];
    		$kegiatan['durasi'] = $value['durasi'];
    		$pendahulu = json_decode($value['pekerjaan_sebelumnya']);
    		...
    		if(count($pendahulu) > 0){
    			foreach ($pendahulu as $k => $v) {
    				$this->setPenerus($v, $value['id']);		
    			}
    		}
    	}
    ...
    /*-------------------------------------------------------------------------*/
    public function setPenerus($key, $penerus)
    {
    	
    	if(isset($this->kegiatan[$key]['penerus'])){
    		$this->kegiatan[$key]['penerus'][] = $penerus;
    	}else{
    		$arr[] = $penerus;	
    		$this->kegiatan[$key]['penerus'] = $arr;
    	}
    }
